...
export class RtmsMediaRecorder {
  ...
  private constructor(
    ...
    // `folderPath` is generated by mkdtemp inside the private create() factory.
    this.audioPath = path.join(folderPath, 'audio.raw'); // nosemgrep: javascript.lang.security.audit.path-traversal.path-join-resolve-traversal.path-join-resolve-traversal
    this.videoPath = path.join(folderPath, 'video.h264'); // nosemgrep: javascript.lang.security.audit.path-traversal.path-join-resolve-traversal.path-join-resolve-traversal
    this.outputPath = path.join(folderPath, 'recording.mp4'); // nosemgrep: javascript.lang.security.audit.path-traversal.path-join-resolve-traversal.path-join-resolve-traversal
  ...
  private static async createBlackFrame(
    ...
    const filePath = path.join(folderPath, 'black-frame.h264'); // nosemgrep: javascript.lang.security.audit.path-traversal.path-join-resolve-traversal.path-join-resolve-traversal
    try {
